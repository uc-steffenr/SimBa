...
class Agent:
    def __init__(self,
        ...
        self.verts = np.concatenate((self.verts.T, np.array([self.verts[0, 0], self.verts[1, 0]]).reshape(1, 2)), axis=0).T
    ...
    def sensor_reading(self,
        ...
        angs = self.angs + X[4]
        sensor_verts = np.array([[X[0] + (self.r - 0.01)*np.cos(angs),
                                  X[2] + (self.r - 0.01)*np.sin(angs)],
